...
Section 1 -- position (IDEAS: the way our model works, females use distnace from past male as a basis for choosing who to visit next, also I might be letting them move too far (.1*circumference)) ONE FIX: make fake mating success in null not truly random but based on simulations with the same positions but no marauding 

Another stray idea (is sd below average the way to go or should the size of a sd be established early on and then stay fixed so that we actually see more fine-tuned changes as (if) mating variance goes down)
```{r}
...
```{r}
#less good IMO!
posvarovergen2 <- all_results %>% group_by(rnd_seed,gen,directory, is_selection, is_scaled) %>% summarise(mate_var=var(successful_mating), quad1=sum(position>=0 & position< male_dist*num_males/3), quad2 = sum(position>= male_dist*num_males/3 & position< 2*male_dist*num_males/3), quad3= sum(position>= 2*male_dist*num_males/5 & position< male_dist*num_males)) %>% mutate(vmr=sum((quad1-2)^2, (quad2-2)^2, (quad3-2)^2)/((2)*(3-1))) %>% ungroup() %>% mutate(z=sqrt((3-1)/(2))*(vmr-1))

mod <- lm(vmr ~ is_scaled + is_selection+mate_var, data=posvarovergen2 %>% filter(gen==1999))
summary(mod)
#forgot to filter the 1999 -- but this isn't exactly a big deal bc things settle very quickly
...
```{r}
#mating variance comparison
model <- lm(mv_after ~ is_selection+male_dist+avg_strat,
              data = stratlow %>% filter(gen==1999))
tab_model(model)
summary(model)

#all * .447 R2 BUT nothing significant
#+ gives .425
...
```{r}
#plot the 3 logistic regression (mating variance)

#the formula for the models
lF<-formula(mv_after~K*n0*exp(r*gen)/(K+n0*(exp(r*gen)-1)) | is_selection)

#stratlow <- stratlow %>% select(-mv_before)

sog300 <- stratlow %>% filter(male_dist==300) 
SS<-getInitial(mv_after~SSlogis(gen,alpha,xmid,scale),data=sog300)
K_start<-SS["alpha"]
R_start<-1/SS["scale"]
N0_start<-SS["alpha"]/(exp(SS["xmid"]/SS["scale"])+1)
#fit the model
(m <- nlsList(lF,data=sog300,start=list(K=K_start,r=R_start, n0=N0_start)))
sog300 <- sog300 %>% arrange(is_selection, rnd_seed)
sog300$pred <- predict(m)
...
sog600 <- stratlow %>% filter(male_dist==600) 
SS<-getInitial(avg_strat~SSlogis(gen,alpha,xmid,scale),data=sog600)
K_start<-SS["alpha"]
R_start<-1/SS["scale"]
N0_start<-SS["alpha"]/(exp(SS["xmid"]/SS["scale"])+1)
#fit the model
(m <- nlsList(lF,data=sog600,start=list(K=K_start,r=R_start, n0=N0_start)))
sog600 <- sog600 %>% arrange(is_selection, rnd_seed)
sog600$pred <- predict(m)
...
sog900 <- stratlow %>% filter(male_dist==900) 
SS<-getInitial(avg_strat~SSlogis(gen,alpha,xmid,scale),data=sog900)
K_start<-SS["alpha"]
R_start<-1/SS["scale"]
N0_start<-SS["alpha"]/(exp(SS["xmid"]/SS["scale"])+1)
#fit the model
(m <- nlsList(lF,data=sog900,start=list(K=K_start,r=R_start, n0=N0_start)))
sog900 <- sog900 %>% arrange(is_selection, rnd_seed)
sog900$pred <- predict(m)
...
pl <- ggplot(data = sog) + aes(x = gen, y = mv_after, color = as.factor(is_selection), group=interaction(rnd_seed,is_selection))  + geom_line( alpha=.2)  + geom_line(aes(y=pred), size=1) + facet_wrap(~male_dist) + labs(x="Generation", y="Mating varaince", title="MV evolution varying density") +  theme_bw() + theme(legend.position="None") + theme(axis.text.x = element_text(angle = 45, hjust=1))
pl
...
```{r}
mod <- lm(mv_after ~ is_selection+is_scaled+male_dist+avg_strat,
              data = filter(stratalt, gen==1999))
#summary(mod)
tab_model(mod)
```
...
pl <- ggplot(data = sog) + aes(x = gen, y = avg_strat, color = as.factor(is_selection), group=interaction(rnd_seed,is_selection))  + geom_line( alpha=.2)  + geom_line(aes(y=pred), size=1) + facet_grid(is_scaled~male_dist) + labs(x="Generation", y="Average strategy", title="Strategy evolution varying density and scaling") +  theme_bw() + theme(legend.position="None") + theme(axis.text.x = element_text(angle = 45, hjust=1))
pl
